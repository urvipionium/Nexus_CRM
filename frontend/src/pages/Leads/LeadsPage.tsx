...
const INITIAL_NOTIFICATIONS = [
  {
    id: 1,
    type: "mention",
    title: "Rahul Mehta mentioned you",
    desc: "In lead #1042 — 'Can you handle this?'",
    time: "2m ago",
    read: false,
    category: "Mentions",
  },
  {
    id: 2,
    type: "system",
    title: "Pipeline value crossed ₹10L",
    desc: "Your pipeline hit a new milestone today.",
    time: "15m ago",
    read: false,
    category: "System",
  },
  {
    ...
    desc: "Priya Sharma was assigned to you by Admin.",
    time: "1h ago",
    read: false,
    category: "All",
  },
  {
    ...
    desc: "Your weekly sales report is ready to download.",
    time: "3h ago",
    read: true,
    category: "System",
  },
  {
    id: 5,
    type: "mention",
    title: "Dev Kumar tagged you",
    desc: "In deal #876 — needs your approval.",
    time: "5h ago",
    read: true,
    category: "Mentions",
  },
  {
    ...
    desc: "Amit Verma moved to Qualified by Sneha Rao.",
    time: "1d ago",
    read: true,
    category: "All",
  },
  {
    id: 7,
    type: "system",
    title: "Two-factor auth reminder",
...
const INITIAL_PROFILES = [
  ...
  },
  {
    ...
    desc: "Standard sales operations",
    created: "2024-01-10",
    users: 2,
    perms: {
...
function Avatar({ name, size = "sm" }) {
  ...
  return (
    <div
      className={`${sz} rounded-xl font-semibold flex items-center justify-center flex-shrink-0 ${getAvatarColor(name)}`}
    >
      {getInitials(name)}
    </div>
  );
...
function StatusBadge({ status }) {
  return (
    <span
      className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_COLORS[status] || "bg-slate-100 text-slate-500"}`}
    >
      {status}
    </span>
  );
...
function NotificationPanel({ onClose }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute right-0 top-full mt-2 w-[380px] bg-white border border-slate-200 rounded-2xl shadow-2xl shadow-slate-200/60 z-[200] flex flex-col overflow-hidden"
      style={{ maxHeight: 520 }}
    >
      {/* Header */}
      ...
      <div className="flex gap-1 px-3 py-2 border-b border-slate-100 flex-shrink-0">
        {cats.map((c) => (
          <button
            key={c}
            onClick={() => setCat(c)}
            className={`px-3 py-1 rounded-lg text-[12px] font-medium transition-all ${cat === c ? "bg-indigo-600 text-white" : "text-slate-500 hover:bg-slate-100"}`}
          >
            {c}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-50">
        ...
        {filtered.map((n) => {
          ...
          return (
            <div
              key={n.id}
              className={`flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors group ${!n.read ? "bg-indigo-50/30" : ""}`}
            >
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${NOTIF_COLORS[n.type] || "bg-slate-100 text-slate-500"}`}
              >
                <NIcon size={14} />
              </div>
              <div className="flex-1 min-w-0">
                <p
                  className={`text-[13px] leading-tight ${!n.read ? "font-semibold text-slate-800" : "font-medium text-slate-600"}`}
                >
                  {n.title}
                </p>
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[100]">
      <div className="bg-white w-[540px] rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
        <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-[12px] text-slate-400 mt-0.5">
              {isEdit ? form.email : "Fill in the user details"}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          {err && (
            <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-[12px] text-rose-600">
              {err}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className={inputCls}
              >
                {["Admin", "Sales Manager", "Sales Exec", "Support Exec"].map(
                  (r) => (
            ...
            <div>
              ...
              <select
                ...
                className={inputCls}
              >
                {["Super Admin", "Manager", "Sales Executive", "Support"].map(
                  (p) => (
            ...
            <div className="flex gap-2">
              {["Active", "Inactive", "Invited"].map((s) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-[12px] font-medium border-2 transition-all ${form.status === s ? "border-indigo-500 bg-indigo-50 text-indigo-600" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
                >
                  {s}
                </button>
              ))}
        ...
        <div className="px-5 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
          ...
          <button
            onClick={handleSave}
            className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-medium flex items-center justify-center gap-2"
          >
            <Check size={14} />
...
function UsersTab() {
  ...
  const saveUser = (data) => {
    if (editingUser)
      setCrmUsers((us) => us.map((u) => (u.id === data.id ? data : u)));
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-white flex-shrink-0">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 text-[13px] rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-indigo-400 w-full"
          />
        </div>
        <div className="flex gap-1.5">
          {statuses.map((s) => (
            <button
              ...
              className={`px-3 py-1.5 text-[12px] font-medium rounded-lg transition-all ${statusFilter === s ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}
            >
              {s}
            </button>
          ))}
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-medium rounded-xl ml-auto shadow-sm transition-all"
        >
      ...
      <div className="flex-1 overflow-y-auto">
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-100 sticky top-0">
            <tr>
              {["User", "Email", "Role", "Profile", "Status", "Actions"].map(
                (h) => (
                  <th
                    key={h}
                    className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider px-6 py-3 text-left"
                  >
                    {h}
                  </th>
                ),
          ...
          <tbody className="divide-y divide-slate-50">
            ...
            {filtered.map((u) => (
              ...
              >
                ...
                <td className="px-6 py-3 text-[13px] text-slate-700 font-medium">
                  {u.role}
                </td>
                <td className="px-6 py-3">
                ...
                <td className="px-6 py-3">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      title="Edit"
                      className="w-7 h-7 rounded-lg hover:bg-sky-50 hover:text-sky-600 text-slate-400 flex items-center justify-center transition-colors"
                    >
                      <Edit3 size={13} />
                    </button>
                    <button
                      ...
                      className="w-7 h-7 rounded-lg hover:bg-amber-50 hover:text-amber-600 text-slate-400 flex items-center justify-center transition-colors"
                    >
                      <UserX size={13} />
                    </button>
                    <button
                      onClick={() => del(u.id)}
                      title="Delete"
                      className="w-7 h-7 rounded-lg hover:bg-rose-50 hover:text-rose-500 text-slate-400 flex items-center justify-center transition-colors"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
...
function ProfilePermMatrix({ profile, onSave, onClose }) {
  ...
  const toggle = (mod, perm) =>
    setPerms((p) => ({
      ...p,
      [mod]: p[mod]?.includes(perm)
        ? p[mod].filter((x) => x !== perm)
        : [...(p[mod] || []), perm],
    }));
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="bg-slate-50 rounded-xl">
                <th className="text-left px-3 py-2 text-slate-500 font-semibold uppercase tracking-wider">
                  Module
                </th>
                {ALL_PERMS.map((p) => (
                  <th
                  ...
                  >
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] ${PERM_COLORS[p]}`}
                    >
                      {p}
                    </span>
                  </th>
                  ...
                  {ALL_PERMS.map((perm) => (
                    <td key={perm} className="px-3 py-2.5 text-center">
                      <button
                        ...
                        className={`w-5 h-5 rounded flex items-center justify-center mx-auto border-2 transition-all ${has(mod, perm) ? "bg-indigo-600 border-indigo-600" : "border-slate-300 hover:border-indigo-400"}`}
                      >
                        {has(mod, perm) && (
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-medium rounded-xl shadow-sm transition-all"
        >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-[11px] font-medium transition-colors"
                    >
                      <Shield size={11} />
                      Permissions
                    </button>
                    <button
                      onClick={() => del(p.id)}
                      className="w-7 h-7 rounded-lg hover:bg-rose-50 flex items-center justify-center transition-colors"
                    >
                      <Trash2
                        size={13}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[100]">
          <div className="bg-white w-[400px] rounded-2xl shadow-2xl">
            ...
            <div className="p-5 space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-indigo-400"
                />
              </div>
              <div>
                <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewProfile({ ...newProfile, desc: e.target.value })
                  }
            ...
            <div className="px-5 pb-5 flex gap-3">
              <button
                onClick={() => setAddModal(false)}
                className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-[13px] font-medium"
              >
                Cancel
              </button>
              <button
                onClick={addProfile}
                className="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white text-[13px] font-medium"
              >
                Add Profile
              </button>
            </div>
...
function RolesTab() {
  ...
  const addRole = () => {
    ...
    setRoles((rs) => [
      ...rs,
      { id: Date.now(), ...form, level: Number(form.level), users: 0 },
    ]);
    setAddModal(false);
    setForm({
      name: "",
  ...
  const inputCls =
    "w-full border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-indigo-400";
  const labelCls =
    "text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5";
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white flex-shrink-0">
        ...
        <button
          onClick={() => setAddModal(true)}
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-medium rounded-xl shadow-sm"
        >
          ...
          <tbody className="divide-y divide-slate-50">
            {roles.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50/50 group">
                ...
                <td className="px-6 py-4">
                  <span
                    className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${levelColors[r.level] || "bg-slate-100 text-slate-500"}`}
                  >
                    L{r.level}
                  </span>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="w-7 h-7 rounded-lg hover:bg-sky-50 flex items-center justify-center">
                      <Edit3
                        size={12}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[100]">
          <div className="bg-white w-[420px] rounded-2xl shadow-2xl">
            ...
            <div className="p-5 space-y-3">
              ...
              <div>
                <label className={labelCls}>Assign Profile</label>
                <select
                  value={form.profile}
                  onChange={(e) =>
...
function ComplianceTab() {
  ...
  const filteredLogs = AUDIT_LOGS.filter(
    (l) =>
      (!auditFilter.user ||
        l.user.toLowerCase().includes(auditFilter.user.toLowerCase())) &&
      (!auditFilter.module ||
        l.module.toLowerCase().includes(auditFilter.module.toLowerCase())),
  );
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="p-6 space-y-5">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          ...
          <SRow label="Minimum Password Length">
            <input
              ...
              min="6"
              max="32"
              value={policies.minPassLen}
              onChange={(e) =>
                setPolicies({ ...policies, minPassLen: Number(e.target.value) })
              }
              className="w-20 border border-slate-200 rounded-lg px-2 py-1 text-[13px] text-center outline-none focus:border-indigo-400"
            />
          </SRow>
          <SRow label="Special Character Required">
            <Toggle
              value={policies.specialChar}
              onChange={(v) => setPolicies({ ...policies, specialChar: v })}
            />
          </SRow>
          <SRow label="Password Expiry (days)">
            <input
              type="number"
              min="0"
              value={policies.passExpiry}
              onChange={(e) =>
                setPolicies({ ...policies, passExpiry: Number(e.target.value) })
              }
              className="w-20 border border-slate-200 rounded-lg px-2 py-1 text-[13px] text-center outline-none focus:border-indigo-400"
            />
          </SRow>
          <SRow label="Two-Factor Authentication">
            <Toggle
              value={policies.twoFA}
              onChange={(v) => setPolicies({ ...policies, twoFA: v })}
            />
          </SRow>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          ...
          <SRow label="Device Restrictions">
            <Toggle
              value={policies.deviceRestrictions}
              onChange={(v) =>
                setPolicies({ ...policies, deviceRestrictions: v })
              }
            />
          </SRow>
        </div>
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) =>
                  setAuditFilter({ ...auditFilter, user: e.target.value })
                }
                className="border border-slate-200 rounded-lg px-2.5 py-1.5 text-[12px] outline-none focus:border-indigo-400 w-36"
              />
              <input
                placeholder="Filter by module…"
                value={auditFilter.module}
                onChange={(e) =>
                  setAuditFilter({ ...auditFilter, module: e.target.value })
                }
                className="border border-slate-200 rounded-lg px-2.5 py-1.5 text-[12px] outline-none focus:border-indigo-400 w-36"
              />
            </div>
            ...
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                {["User", "Action", "Module", "Date", "IP Address"].map((h) => (
                  <th
                    key={h}
                    className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider px-5 py-2.5 text-left"
                  >
                    {h}
                  </th>
                ))}
        ...
        <button className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-medium transition-all flex items-center justify-center gap-2 shadow-sm shadow-indigo-200">
          <Check size={14} /> Save Compliance Settings
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <SectionHeader
        ...
        subtitle="Manage team members, roles, profiles and compliance"
      />
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center">
        <EmptyState
          ...
          desc="This module is coming soon. Stay tuned!"
        />
      </div>
    </div>
  );
...
function SettingsSidebar({ active, onChange, collapsed, onToggle }) {
  ...
  return (
    <div
      className={`flex flex-col bg-white border-r border-slate-100 transition-all duration-200 flex-shrink-0 ${collapsed ? "w-16" : "w-60"} overflow-hidden`}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-4 border-b border-slate-100 flex-shrink-0">
        {!collapsed && (
          <p className="text-[13px] font-bold text-slate-800">Settings</p>
        )}
        <button
          onClick={onToggle}
          className={`w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center flex-shrink-0 transition-colors ${collapsed ? "mx-auto" : ""}`}
        >
          {collapsed ? (
          ...
          <div key={section.id}>
            {section.hasChildren ? (
                ...
                >
                  ...
                  {!collapsed && (
                    <>
                      ...
                      <ChevronDown
                        size={12}
                        className={`text-slate-400 transition-transform ${expanded[section.id] ? "rotate-180" : ""}`}
                      />
                    </>
                  )}
                </button>
              ...
              >
                <section.icon
                  size={16}
                  className={`flex-shrink-0 ${active === section.id ? "text-indigo-600" : "text-slate-500"}`}
                />
                {!collapsed && (
                  <span
                    className={`text-[13px] font-medium ${active === section.id ? "text-indigo-700" : "text-slate-600"}`}
                  >
                    {section.label}
                  </span>
                )}
...
function SettingsPage({ onBack }) {
      ...
      case "fields":
        return (
          <PlaceholderPage
            icon={FileText}
            title="Fields"
            desc="Manage custom field definitions"
          />
        );
      case "stages":
        return (
          <PlaceholderPage
            icon={Layers}
            title="Stages"
            desc="Configure pipeline stages"
          />
        );
      case "forms":
        return (
          <PlaceholderPage
            icon={FileText}
            title="Forms"
            desc="Build and manage web forms"
          />
        );
      case "automation":
        return (
          <PlaceholderPage
            icon={Zap}
            title="Automation"
            desc="Create workflows and automation rules"
          />
        );
      case "data":
        return (
          <PlaceholderPage
            ...
            desc="Import, export and manage your data"
          />
        );
      case "toppings":
        return (
          <PlaceholderPage
            icon={Sliders}
            title="Toppings"
            desc="Extra configurations and integrations"
          />
        );
      case "trial":
        return (
          <PlaceholderPage
            icon={Gift}
            title="Free Trial"
            desc="Manage your trial and subscription"
          />
        );
      case "channels-email":
        return (
          <PlaceholderPage
            icon={Mail}
            title="Email Channel"
            desc="Configure email sending and templates"
          />
        );
      case "channels-messages":
        return (
          <PlaceholderPage
            icon={MessageSquare}
            title="Messages"
            desc="Configure SMS and chat messages"
          />
        );
      case "channels-phone":
        return (
          <PlaceholderPage
            icon={PhoneCall}
            title="Phone Channel"
            desc="Configure call center and phone settings"
          />
        );
      case "channels-social":
        return (
          <PlaceholderPage
            ...
            desc="Connect Facebook, Instagram and more"
          />
        );
      default:
        return <UsersAndControlsPage />;
  ...
  return (
    <div className="h-screen bg-[#F7F8FC] flex flex-col overflow-hidden font-sans">
      {/* Top bar */}
      <header
        className="bg-white border-b border-slate-100 px-6 flex items-center gap-4 flex-shrink-0"
        style={{ height: 68 }}
      >
        <button
          onClick={onBack}
        ...
        >
          <ChevronLeft size={18} />
          <span className="text-[13px] font-medium">Back to Leads</span>
        </button>
      ...
      {open && (
        <div className="absolute top-full mt-1 left-0 z-[60] bg-white border border-slate-200 rounded-2xl shadow-2xl p-2 w-[210px]">
          {FIELD_TYPE_OPTIONS.map(({ type, label, icon: FIcon, desc }) => (
            ...
            >
              <div
                className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${value === type ? "bg-indigo-100" : "bg-slate-100"}`}
              >
                <FIcon
                  size={13}
                  className={
                    value === type ? "text-indigo-600" : "text-slate-500"
                  }
                />
              </div>
              ...
              {value === type && (
                <Check
                  size={12}
                  className="ml-auto text-indigo-500 flex-shrink-0"
                />
              )}
            </button>
          ))}
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-3 bg-slate-50 space-y-2.5">
      <div className="flex items-center gap-2">
        <GripVertical
          size={14}
          className="text-slate-300 flex-shrink-0 cursor-grab"
        />
        <input
        ...
        <TypePicker
          ...
          onChange={(t) =>
            onUpdate({
              ...field,
              type: t,
              options: t === "select" ? ["Option 1"] : undefined,
            })
          }
        />
        <button
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && newOpt.trim()) {
                  onUpdate({
                    ...field,
                    options: [...(field.options || []), newOpt.trim()],
                  });
                  setNewOpt("");
                }
              }}
    ...
    <div className="p-5 space-y-3 max-h-[420px] overflow-y-auto">
      <div className="mb-1">
        ...
        <p className="text-[11px] text-slate-400 mt-0.5">
          Rename, recolor, add or remove stages
        </p>
      </div>
      {stages.map((s) => (
        <div
        ...
        >
          <GripVertical
            size={14}
            className="text-slate-300 cursor-grab flex-shrink-0"
          />
          <span
      ...
      <button
        ...
        onClick={() => {
          ...
          const nextPalette =
            STAGE_PALETTE.find((p) => !usedColors.includes(p.color)) ||
            STAGE_PALETTE[0];
          setStages([
            ...stages,
            { id: uid(), label: "New Stage", ...nextPalette },
          ]);
        }}
      ...
      <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl">
        <p className="text-[11px] text-amber-700 leading-relaxed">
          <strong>Note:</strong> Stage changes apply globally. Leads in a
          removed stage stay there until manually moved.
        </p>
      </div>
    </div>
  );
...
function NewLeadModal({
  ...
  const updateFieldDef = (id, updated) =>
    setCustomFields(customFields.map((f) => (f.id === id ? updated : f)));
  const deleteFieldDef = (id) =>
    setCustomFields(customFields.filter((f) => f.id !== id));
  const addFieldDef = (type) => {
    setCustomFields([
      ...customFields,
      {
        id: uid(),
  ...
  const tabCls = (t) =>
    `px-3 py-1.5 text-[12px] font-medium rounded-lg transition-all ${tab === t ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-100"}`;
  const inputCls =
    "w-full border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all";
  const labelCls =
    "text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5";
                ...
                {customFields.map((field) => (
                  <div key={field.id}>
                    ...
                    <CustomFieldInput
                      ...
                      onChange={(v) =>
                        setCustomValues({ ...customValues, [field.id]: v })
                      }
                    />
                  </div>
                ))}
        ...
        <div className="p-6">
          <div className="p-4 bg-rose-50 border border-rose-100 rounded-xl mb-4">
            <p className="text-[13px] text-rose-700 leading-relaxed">
              <strong>{lead.name}</strong> ({lead.company}) will be permanently
              deleted. This action cannot be undone.
            </p>
          </div>
          <p className="text-[13px] text-slate-500">
            Deal value of <strong>{formatVal(lead.value)}</strong> will be
        ...
        <div className="p-5 space-y-3">
          ...
          {leadCount > 0 ? (
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <div>
                ...
                <p className="text-[12px] text-amber-700 leading-relaxed">
                  Pipeline value of <strong>{formatVal(leadValue)}</strong> will
                  be lost. All leads in this stage will be permanently deleted.
                </p>
              </div>
            </div>
          ) : (
            <div className="flex items-start gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl">
              ...
              />
              <p className="text-[13px] text-slate-500">
                This stage is empty. It will be safely removed from your
                pipeline.
              </p>
            </div>
          )}
...
function LeadsPage({ onOpenSettings }) {
  ...
  const onDrop = (e, stageId) => {
    ...
    setLeads((prev) =>
      prev.map((l) => (l.id === Number(id) ? { ...l, stage: stageId } : l)),
    );
    setDraggingId(null);
  };
  ...
  const moveLead = (leadId, newStageId) => {
    setLeads((prev) =>
      prev.map((l) => (l.id === leadId ? { ...l, stage: newStageId } : l)),
    );
    setSelectedLead((prev) => (prev ? { ...prev, stage: newStageId } : prev));
  };
  const getStageById = (id) => stages.find((s) => s.id === id);
  const openNewLead = (defaultStage) => {
  ...
  const handleDeleteStage = () => {
    ...
    if (selectedLead && selectedLead.stage === deletingStage.id)
    ...
    <div className="h-screen bg-[#F7F8FC] flex flex-col overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-[13px] rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 w-[200px] transition-all"
            />
          </div>
          {/* Bell */}
          <div className="relative" ref={notifRef}>
            <button
              ...
              className="relative w-9 h-9 rounded-xl bg-slate-50 border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 flex items-center justify-center transition-all"
            >
              <Bell
                size={16}
                className={showNotifs ? "text-indigo-600" : "text-slate-500"}
              />
              {unreadCount > 0 && (
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl bg-slate-50 border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 flex items-center justify-center transition-all"
            title="Settings"
          >
            <Settings
              size={16}
      ...
      <div className="flex gap-4 px-8 py-3 bg-white border-b border-slate-100 flex-shrink-0">
        {[
          ...
          <div
            key={label}
            className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-2.5 flex-1"
          >
            <div
                ...
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{ background: stage.color }}
                    />
                    <h2 className="font-semibold text-[14px] text-slate-700">
                      {stage.label}
                    </h2>
                  ...
                  <div className="flex items-center gap-1">
                    {stages.length > 2 && (
                      <button
                        ...
                        title="Delete stage"
                        className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-150 ${isHovered ? "opacity-100 bg-rose-50 border border-rose-200 hover:bg-rose-100" : "opacity-0 pointer-events-none"}`}
                      >
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                ...
                {colLeads.map((lead) => (
                  <div
                    ...
                    className={`bg-white border rounded-xl p-4 cursor-pointer transition-all select-none relative ${draggingId === lead.id ? "opacity-40 scale-95 border-indigo-300" : "border-slate-200 hover:border-indigo-300 hover:shadow-md hover:shadow-indigo-50"}`}
                  >
                    <div
                    ...
                    <div className="flex items-start justify-between mb-3 pr-20">
                      ...
                      <span
                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 absolute top-4 right-4 transition-opacity ${hoveredCardId === lead.id ? "opacity-0" : "opacity-100"} ${TAG_COLORS[lead.tag] || "bg-slate-100 text-slate-500"}`}
                      >
                        {lead.tag}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                      ...
                      <div className="flex items-center gap-1 text-[11px]">
                        <Clock
                          size={11}
                          className={
                            lead.aging > 7 ? "text-rose-400" : "text-slate-300"
                          }
                        />
                        <span
                          className={
            ...
            <div className="px-6 pt-6 pb-4 border-b border-slate-100 flex-shrink-0">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-[17px] text-slate-800">
                      {selectedLead.name}
                    </h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    label: "Deal Value",
                    value: formatVal(selectedLead.value),
                    icon: DollarSign,
                  },
                  {
      ...
      {deletingStage && (
        <DeleteStageModal
          ...
          leadValue={leads
            .filter((l) => l.stage === deletingStage.id)
            .reduce((s, l) => s + l.value, 0)}
...
export default function App() {
  ...
  const [localLeads, setLeads] = useState([]);
  ...
  return (
    ...
    >
      {page === "leads" && (
        <LeadsPage onOpenSettings={() => setPage("settings")} />
      )}
