      ...
      <div className="bg-white w-[420px] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[16px] font-bold text-slate-800">Delete Stage?</h2>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-slate-200 bg-slate-50">
            ...
            <span className="text-[14px] font-semibold text-slate-700">{stage.label}</span>
            <span className="ml-auto text-[12px] text-slate-400">{leadCount} lead{leadCount !== 1 ? "s" : ""}</span>
          </div>

          {/* Warning — changes based on whether stage has leads */}
          {leadCount > 0 ? (
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              <AlertTriangle size={15} className="text-amber-500 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-[13px] font-semibold text-amber-800 leading-tight mb-1">
                  This stage has {leadCount} lead{leadCount !== 1 ? "s" : ""}
                </p>
                <p className="text-[12px] text-amber-700 leading-relaxed">
                  Pipeline value of <strong>{formatVal(leadValue)}</strong> will be lost. All leads in
                  this stage will be permanently deleted along with the stage.
                </p>
              </div>
            </div>
          ) : (
            <div className="flex items-start gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl">
              <Check size={15} className="text-slate-400 flex-shrink-0 mt-0.5" />
              <p className="text-[13px] text-slate-500">
                This stage is empty. It will be safely removed from your pipeline.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="px-5 pb-5 flex gap-3">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-[13px] font-semibold transition-all flex items-center justify-center gap-2 shadow-lg shadow-rose-100"
          >
...
// ─────────────────────────────────────────────────────────
// SUB-COMPONENT: Field Type Picker
// ─────────────────────────────────────────────────────────
        ...
        <div className="absolute top-full mt-1 left-0 z-[60] bg-white border border-slate-200 rounded-2xl shadow-2xl p-2 w-[210px]">
          {FIELD_TYPE_OPTIONS.map(({ type, label, icon: FIcon, desc }) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-2.5 py-2 rounded-xl text-left transition-colors ${value === type ? "bg-indigo-50 text-indigo-700" : "hover:bg-slate-50 text-slate-700"}`}
            >
...
// ─────────────────────────────────────────────────────────
// SUB-COMPONENT: Field Def Row
// ─────────────────────────────────────────────────────────

function FieldDefRow({ field, onUpdate, onDelete }: { field: CustomFieldDef; onUpdate: (f: CustomFieldDef) => void; onDelete: () => void }) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-3 bg-slate-50 space-y-2.5">
      <div className="flex items-center gap-2">
        ...
        <input placeholder="Field label…" value={field.label} onChange={(e) => onUpdate({ ...field, label: e.target.value })}
        ...
        <button type="button" onClick={onDelete} className="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center hover:border-rose-300 hover:bg-rose-50 transition-colors flex-shrink-0">
        ...
        <div className="pl-5 space-y-1.5">
          {(field.options || []).map((opt, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <input value={opt} onChange={(e) => { const opts = [...(field.options || [])]; opts[i] = e.target.value; onUpdate({ ...field, options: opts }); }}
                className="flex-1 text-[12px] bg-white border border-slate-200 rounded-lg px-2 py-1 outline-none focus:border-indigo-300" />
              <button type="button" onClick={() => onUpdate({ ...field, options: (field.options || []).filter((_, j) => j !== i) })} className="text-slate-300 hover:text-rose-400 transition-colors">
                <X size={11} />
              </button>
            </div>
          ))}
          <div className="flex items-center gap-2 mt-1">
            <input placeholder="New option…" value={newOpt} onChange={(e) => setNewOpt(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter" && newOpt.trim()) { onUpdate({ ...field, options: [...(field.options || []), newOpt.trim()] }); setNewOpt(""); } }}
            ...
            <button type="button" onClick={() => { if (newOpt.trim()) { onUpdate({ ...field, options: [...(field.options || []), newOpt.trim()] }); setNewOpt(""); } }}
              className="text-[11px] text-indigo-500 hover:text-indigo-700 font-medium">+ Add</button>
          </div>
        </div>
      )}
    </div>
...
// ─────────────────────────────────────────────────────────
// SUB-COMPONENT: Custom Field Input
// ─────────────────────────────────────────────────────────

function CustomFieldInput({ field, value, onChange }: { field: CustomFieldDef; value: any; onChange: (v: any) => void }) {
...
// ─────────────────────────────────────────────────────────
// SUB-COMPONENT: Stage Manager Panel
// ─────────────────────────────────────────────────────────

function StageManagerPanel({ stages, setStages }: { stages: PipelineStage[]; setStages: (s: PipelineStage[]) => void }) {
  return (
    <div className="p-5 space-y-3 max-h-[420px] overflow-y-auto">
      ...
      {stages.map((s) => (
        <div key={s.id} className="flex items-center gap-3 bg-slate-50 rounded-xl px-3 py-2.5 border border-slate-200">
          ...
          <input value={s.label} onChange={(e) => setStages(stages.map((x) => (x.id === s.id ? { ...x, label: e.target.value } : x)))}
            className="flex-1 min-w-0 text-[13px] font-medium bg-transparent outline-none text-slate-700" />
          <div className="flex gap-1">
            {STAGE_PALETTE.map((p, i) => (
              <button key={i} type="button" title={p.color} onClick={() => setStages(stages.map((x) => (x.id === s.id ? { ...x, ...p } : x)))}
                className="w-4 h-4 rounded-full border-2 transition-all flex-shrink-0"
                style={{ background: p.color, borderColor: s.color === p.color ? "#1e293b" : "transparent" }} />
            ))}
          </div>
          {stages.length > 2 && (
            <button type="button" onClick={() => setStages(stages.filter((x) => x.id !== s.id))}
      ...
      <button type="button"
        onClick={() => { const usedColors = stages.map((s) => s.color); const nextPalette = STAGE_PALETTE.find((p) => !usedColors.includes(p.color)) || STAGE_PALETTE[0]; setStages([...stages, { id: uid(), label: "New Stage", ...nextPalette }]); }}
      ...
      <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl">
        <p className="text-[11px] text-amber-700 leading-relaxed">
          <strong>Note:</strong> Stage changes apply globally. Leads in a removed stage stay there until manually moved.
        </p>
      </div>
    </div>
  );
...
function NewLeadModal({ onClose, onSave, stages, setStages, customFields, setCustomFields, defaultStage }: {
  onClose: () => void; onSave: (lead: Omit<Lead, "id">) => void;
  stages: PipelineStage[]; setStages: (s: PipelineStage[]) => void;
  customFields: CustomFieldDef[]; setCustomFields: (f: CustomFieldDef[]) => void;
  defaultStage?: string;
}) {
  ...
  const updateFieldDef = (id: string, updated: CustomFieldDef) => setCustomFields(customFields.map((f) => (f.id === id ? updated : f)));
  const deleteFieldDef = (id: string) => setCustomFields(customFields.filter((f) => f.id !== id));
  const addFieldDef = (type: FieldType) => { setCustomFields([...customFields, { id: uid(), label: "", type, options: type === "select" ? ["Option 1"] : undefined }]); setShowFieldPicker(false); setTab("form"); };
  ...
  const tabCls = (t: string) => `px-3 py-1.5 text-[12px] font-medium rounded-lg transition-all ${tab === t ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-100"}`;
      ...
      <div className="bg-white w-[520px] rounded-2xl shadow-2xl overflow-visible flex flex-col max-h-[90vh]">
        <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center justify-between mb-4">
            <div><h2 className="text-[18px] font-bold text-slate-800">New Lead</h2><p className="text-[12px] text-slate-400 mt-0.5">Fill in the details below</p></div>
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><X size={15} className="text-slate-500" /></button>
          </div>
          <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1">
            ...
            <button className={tabCls("fields")} onClick={() => setTab("fields")}>
              <span className="flex items-center gap-1.5"><Settings2 size={11} />Custom Fields{customFields.length > 0 && <span className="w-4 h-4 rounded-full bg-indigo-100 text-indigo-600 text-[10px] font-bold flex items-center justify-center">{customFields.length}</span>}</span>
            </button>
          ...
          <div className="p-5 space-y-4 overflow-y-auto flex-1">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {stages.map((s) => (<button key={s.id} type="button" onClick={() => setForm({ ...form, stage: s.id })} className="px-3 py-1.5 rounded-xl text-[12px] font-semibold border-2 transition-all" style={form.stage === s.id ? { background: s.color, borderColor: s.color, color: "#fff" } : { background: "#fff", borderColor: "#e2e8f0", color: "#64748b" }}>{s.label}</button>))}
                <button type="button" onClick={() => setTab("stages")} className="px-3 py-1.5 rounded-xl text-[12px] font-medium border-2 border-dashed border-slate-200 text-slate-400 hover:border-indigo-300 hover:text-indigo-500 transition-all flex items-center gap-1"><Plus size={11} /> Stage</button>
              </div>
            </div>
            {customFields.length > 0 && (
              <div className="space-y-3 pt-2 border-t border-slate-100">
                <p className={labelCls}>Custom Fields</p>
                {customFields.map((field) => (<div key={field.id}><label className={labelCls}>{field.label || <span className="italic text-slate-300">Unnamed field</span>}</label><CustomFieldInput field={field} value={customValues[field.id]} onChange={(v) => setCustomValues({ ...customValues, [field.id]: v })} /></div>))}
              </div>
            )}
                ...
                <div className="absolute bottom-full mb-2 left-0 right-0 bg-white border border-slate-200 rounded-2xl shadow-2xl p-3 z-[60]">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {FIELD_TYPE_OPTIONS.map(({ type, label, icon: Icon }) => (
                      <button key={type} type="button" onClick={() => addFieldDef(type)} className="flex flex-col items-center gap-1.5 p-2.5 rounded-xl hover:bg-indigo-50 border border-transparent hover:border-indigo-100 transition-all group">
                        <div className="w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-indigo-100 flex items-center justify-center transition-colors"><Icon size={15} className="text-slate-500 group-hover:text-indigo-600" /></div>
                        <span className="text-[11px] font-medium text-slate-600 group-hover:text-indigo-700 leading-tight text-center">{label}</span>
                      </button>
                    ))}
                  </div>
                </div>
        ...
        <div className="px-5 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-[13px] font-medium hover:bg-slate-50 transition-colors">Cancel</button>
...
function EditLeadModal({ lead, stages, customFields, onClose, onSave }: { lead: Lead; stages: PipelineStage[]; customFields: CustomFieldDef[]; onClose: () => void; onSave: (updated: Lead) => void }) {
  const inputCls = "w-full border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all";
  const labelCls = "text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5";
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white w-[520px] rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex-shrink-0 flex items-center justify-between">
          <div><h2 className="text-[18px] font-bold text-slate-800">Edit Lead</h2><p className="text-[12px] text-slate-400 mt-0.5">{lead.name}</p></div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><X size={15} className="text-slate-500" /></button>
        </div>
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>Lead Name *</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} /></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>Deal Value (₹)</label><input type="number" value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} className={inputCls} /></div>
            <div><label className={labelCls}>Owner</label><input value={form.owner} onChange={(e) => setForm({ ...form, owner: e.target.value })} className={inputCls} /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>Win Probability (%)</label><input type="number" min="0" max="100" value={form.probability} onChange={(e) => setForm({ ...form, probability: e.target.value })} className={inputCls} /></div>
            <div><label className={labelCls}>Lead Tag</label><select value={form.tag} onChange={(e) => setForm({ ...form, tag: e.target.value })} className={inputCls}>{["Hot","Warm","Cold","Closed"].map((t) => <option key={t}>{t}</option>)}</select></div>
          </div>
          <div>
            <label className={labelCls}>Stage</label>
            <div className="flex flex-wrap gap-2">
        ...
        <div className="px-5 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-[13px] font-medium hover:bg-slate-50 transition-colors">Cancel</button>
          <button type="button" onClick={handleSave} disabled={!form.name || !form.phone} className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white text-[13px] font-medium transition-all flex items-center justify-center gap-2"><Check size={14} /> Save Changes</button>
        </div>
      </div>
    </div>
  );
...
// ─────────────────────────────────────────────────────────
// SUB-COMPONENT: Delete Lead Confirm
// ─────────────────────────────────────────────────────────

function DeleteConfirmModal({ lead, onClose, onConfirm }: { lead: Lead; onClose: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white w-[400px] rounded-2xl shadow-2xl flex flex-col">
        <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-rose-50 flex items-center justify-center"><AlertTriangle size={15} className="text-rose-500" /></div><h2 className="text-[16px] font-bold text-slate-800">Delete Lead</h2></div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><X size={15} className="text-slate-500" /></button>
        </div>
        <div className="p-6">
          <div className="p-4 bg-rose-50 border border-rose-100 rounded-xl mb-4"><p className="text-[13px] text-rose-700 leading-relaxed"><strong>{lead.name}</strong> ({lead.company}) will be permanently deleted. This action cannot be undone.</p></div>
          <p className="text-[13px] text-slate-500">Deal value of <strong>{formatVal(lead.value)}</strong> will be removed from your pipeline.</p>
        </div>
        <div className="px-5 py-4 border-t border-slate-100 flex gap-3">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-[13px] font-medium hover:bg-slate-50 transition-colors">Cancel</button>
          <button type="button" onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-[13px] font-medium transition-all flex items-center justify-center gap-2"><Trash2 size={14} /> Delete Lead</button>
        </div>
      </div>
    </div>
  );
...
function UserPermissionsModal({ users, setUsers, onClose }: { users: TeamUser[]; setUsers: (u: TeamUser[]) => void; onClose: () => void }) {
  const [inviteEmail, setInviteEmail] = useState("");
  const [inviteRole, setInviteRole] = useState<"Admin"|"Edit"|"View">("View");
  const inputCls = "border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all";
  const labelCls = "text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-1.5";
  const handleInvite = () => { if (!inviteEmail.trim()) return; const name = inviteEmail.split("@")[0].replace(/[._]/g," ").replace(/\b\w/g,(c)=>c.toUpperCase()); setUsers([...users,{id:Date.now(),name,email:inviteEmail.trim(),role:inviteRole,active:true}]); setInviteEmail(""); };

  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white w-[540px] rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex-shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center"><UserCog size={18} className="text-indigo-600" /></div><div><h2 className="text-[17px] font-bold text-slate-800">User Permissions</h2><p className="text-[12px] text-slate-400 mt-0.5">Manage team access to this pipeline</p></div></div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors"><X size={15} className="text-slate-500" /></button>
        </div>
        <div className="p-5 overflow-y-auto flex-1 space-y-5">
          <div><p className={labelCls}>Invite Team Member</p>
            <div className="flex gap-2">
              <input placeholder="Email address" value={inviteEmail} onChange={(e)=>setInviteEmail(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&handleInvite()} className={inputCls+" flex-1"} />
              <select value={inviteRole} onChange={(e)=>setInviteRole(e.target.value as any)} className={inputCls+" w-[100px]"}>{["Admin","Edit","View"].map(r=><option key={r}>{r}</option>)}</select>
              <button onClick={handleInvite} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-medium transition-all"><Send size={13} /> Invite</button>
            </div>
          </div>
          <div className="flex gap-3 p-3 bg-slate-50 rounded-xl">
            {[{role:"Admin",desc:"Full access",icon:Shield},{role:"Edit",desc:"Add & edit leads",icon:Pencil},{role:"View",desc:"Read-only",icon:Eye}].map(({role,desc})=>(
              <div key={role} className="flex items-center gap-2 flex-1"><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${ROLE_COLORS[role]}`}>{role}</span><span className="text-[11px] text-slate-400">{desc}</span></div>
            ))}
          </div>
          <div><p className={labelCls}>Team Members ({users.length})</p>
            <div className="space-y-2">
              {users.map(user=>(
                <div key={user.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${user.active?"border-slate-200 bg-white":"border-slate-100 bg-slate-50 opacity-60"}`}>
                  <div className={`w-9 h-9 rounded-xl text-[12px] font-semibold flex items-center justify-center flex-shrink-0 ${getAvatarColor(user.name)}`}>{getInitials(user.name)}</div>
                  <div className="flex-1 min-w-0"><p className="text-[13px] font-semibold text-slate-800 leading-tight">{user.name}</p><p className="text-[11px] text-slate-400 truncate">{user.email}</p></div>
                  <select value={user.role} onChange={(e)=>setUsers(users.map(u=>u.id===user.id?{...u,role:e.target.value as any}:u))} className="text-[12px] font-medium border border-slate-200 rounded-lg px-2 py-1 outline-none focus:border-indigo-400 transition-all bg-white">{["Admin","Edit","View"].map(r=><option key={r}>{r}</option>)}</select>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ${ROLE_COLORS[user.role]}`}>{user.role}</span>
                  <button onClick={()=>setUsers(users.map(u=>u.id===user.id?{...u,active:!u.active}:u))} className={`w-10 h-5 rounded-full transition-colors relative flex-shrink-0 ${user.active?"bg-indigo-500":"bg-slate-200"}`} title={user.active?"Deactivate":"Activate"}>
                  ...
                  <button onClick={()=>setUsers(users.filter(u=>u.id!==user.id))} className="w-7 h-7 rounded-lg hover:bg-rose-50 hover:border-rose-200 border border-transparent flex items-center justify-center transition-colors flex-shrink-0" title="Remove user"><X size={13} className="text-slate-400 hover:text-rose-500"/></button>
                </div>
              ))}
            </div>
          </div>
        </div>
        <div className="px-5 py-4 border-t border-slate-100 flex-shrink-0"><button onClick={onClose} className="w-full py-2.5 rounded-xl border border-slate-200 text-slate-600 text-[13px] font-medium hover:bg-slate-50 transition-colors">Close</button></div>
      </div>
    </div>
  );
}
...
export default function LeadsPage() {
  ...
  // ── Drag & Drop ──
  const onDragStart = (e: React.DragEvent, id: number) => { e.dataTransfer.setData("id", String(id)); setDraggingId(id); };
  const onDragEnd = () => setDraggingId(null);
  const onDrop = (e: React.DragEvent, stageId: string) => { const id = e.dataTransfer.getData("id"); setLeads(prev => prev.map(l => l.id === Number(id) ? { ...l, stage: stageId } : l)); setDraggingId(null); };

  // ── CRUD ──
  const handleCreateLead = (data: Omit<Lead, "id">) => { setLeads([...leads, { ...data, id: Date.now() }]); setShowModal(false); };
  const handleEditSave = (updated: Lead) => { setLeads(prev => prev.map(l => l.id === updated.id ? updated : l)); if (selectedLead?.id === updated.id) setSelectedLead(updated); setEditingLead(null); };
  ...
  const moveLead = (leadId: number, newStageId: string) => { setLeads(prev => prev.map(l => l.id === leadId ? { ...l, stage: newStageId } : l)); setSelectedLead(prev => prev ? { ...prev, stage: newStageId } : prev); };
  const getStageById = (id: string) => stages.find((s) => s.id === id);
  const openNewLead = (defaultStage?: string) => { setNewLeadDefaultStage(defaultStage); setShowModal(true); };

  // ── Delete Stage ──
  const handleDeleteStage = () => {
    if (!deletingStage) return;
    // Remove all leads in this stage
    setLeads(prev => prev.filter(l => l.stage !== deletingStage.id));
    // Remove the stage
    setStages(stages.filter(s => s.id !== deletingStage.id));
    // Clear selected lead if it was in this stage
    if (selectedLead && selectedLead.stage === deletingStage.id) setSelectedLead(null);
    ...
    <div className="h-screen bg-[#F7F8FC] flex flex-col overflow-hidden font-sans">
      ...
      <header className="bg-white border-b border-slate-100 px-8 flex items-center justify-between flex-shrink-0" style={{ height: 68 }}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-semibold text-slate-800 tracking-tight leading-none">Leads Pipeline</h1>
            <p className="text-[12px] text-slate-400 mt-0.5">Drag cards to update stages</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => setShowPermissions(true)} className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 text-[13px] font-medium transition-all" title="User Permissions">
            <UserCog size={15} /><span className="hidden sm:inline">Permissions</span>
            {users.length > 0 && <span className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-600 text-[10px] font-bold flex items-center justify-center">{users.filter(u => u.active).length}</span>}
          </button>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={15} />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search leads…" className="pl-9 pr-4 py-2 text-[13px] rounded-xl bg-slate-50 border border-slate-200 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 w-[220px] transition-all" />
          </div>
      ...
      <div className="flex gap-5 px-8 py-5 overflow-x-auto flex-1 min-h-0">
        {stages.map((stage) => {
          ...
          return (
            ...
            >
              {/* ── Column header with hover-reveal delete ── */}
              <div
                className="px-4 pt-4 pb-3 border-b border-slate-100 flex-shrink-0 relative"
                onMouseEnter={() => setHoveredColId(stage.id)}
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-1">
                    {/* Delete stage button — fades in on hover, hidden if only 2 stages left */}
                    {stages.length > 2 && (
                      <button
                        ...
                        className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-150 ${
                          isHovered
                            ? "opacity-100 bg-rose-50 border border-rose-200 hover:bg-rose-100"
                ...
                {colLeads.map((lead) => (
                  <div
                    ...
                    className={`bg-white border rounded-xl p-4 cursor-pointer transition-all select-none relative ${
                      draggingId === lead.id ? "opacity-40 scale-95 border-indigo-300" : "border-slate-200 hover:border-indigo-300 hover:shadow-md hover:shadow-indigo-50"
                    }`}
                  >
                    {/* Card action buttons */}
                    <div
                      className={`absolute top-2.5 right-2.5 flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-1 shadow-sm transition-all z-10 ${hoveredCardId === lead.id ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-1 pointer-events-none"}`}
                    ...
                    <div className="flex items-baseline gap-1 mb-3">
                      <span className="text-[22px] font-bold text-slate-800 leading-none">₹{(lead.value / 1000).toFixed(0)}K</span>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full bg-gradient-to-r ${stage.bar} transition-all`} style={{ width: `${lead.probability}%` }} />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                      <div className="flex items-center gap-1.5">
                        <div className={`w-5 h-5 rounded-full text-[9px] font-bold flex items-center justify-center ${getAvatarColor(lead.owner)}`}>{lead.owner[0]}</div>
                      ...
                      <div className="flex items-center gap-1 text-[11px]">
                        ...
                        <span className={lead.aging > 7 ? "text-rose-500 font-medium" : "text-slate-400"}>{lead.aging}d</span>
                      </div>
                    </div>
          ...
          <div className="fixed right-0 top-0 w-[390px] h-full bg-white border-l border-slate-200 shadow-2xl flex flex-col z-50">
            <div className="px-6 pt-6 pb-4 border-b border-slate-100 flex-shrink-0">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-2xl text-[16px] font-bold flex items-center justify-center ${getAvatarColor(selectedLead.name)}`}>{getInitials(selectedLead.name)}</div>
                  <div><h2 className="font-bold text-[17px] text-slate-800">{selectedLead.name}</h2><p className="text-[12px] text-slate-400">{selectedLead.company}</p></div>
                </div>
                <div className="flex items-center gap-1.5">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              <div className="rounded-xl bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 p-4">
                <div className="flex items-center gap-2 mb-2"><Sparkles size={13} className="text-indigo-500" /><span className="text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">AI Insight</span></div>
                <p className="text-[13px] text-slate-700 leading-relaxed">
                  {selectedLead.probability >= 70 ? "High-value lead with strong close probability. Prioritize follow-up today." : selectedLead.aging > 7 ? `Lead has been inactive for ${selectedLead.aging} days. Send a re-engagement message.` : "Steady pipeline lead. Schedule a discovery call to qualify further."}
                </p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                {[{label:"Deal Value",value:formatVal(selectedLead.value),icon:DollarSign},{label:"Win Probability",value:`${selectedLead.probability}%`,icon:TrendingUp},{label:"Lead Age",value:`${selectedLead.aging} days`,icon:Clock},{label:"Owner",value:selectedLead.owner,icon:Users}].map(({label,value,icon:Icon})=>(
                  <div key={label} className="bg-slate-50 rounded-xl p-3"><div className="flex items-center gap-1.5 mb-1"><Icon size={11} className="text-slate-400" /><p className="text-[11px] text-slate-400">{label}</p></div><p className="text-[14px] font-semibold text-slate-800">{value}</p></div>
                ))}
              </div>

              <div className="bg-slate-50 rounded-xl p-4">
                <p className="text-[11px] text-slate-400 uppercase tracking-wider font-medium mb-3">Contact</p>
                <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-slate-200 flex items-center justify-center"><Phone size={13} className="text-slate-500" /></div><span className="text-[13px] font-medium text-slate-700">{selectedLead.phone}</span></div>
              </div>

              {customFields.length > 0 && selectedLead.customValues && (
                <div className="bg-slate-50 rounded-xl p-4">
                  <p className="text-[11px] text-slate-400 uppercase tracking-wider font-medium mb-3">Custom Fields</p>
                  <div className="space-y-2">
                    {customFields.map(cf=><div key={cf.id} className="flex justify-between items-start gap-3"><span className="text-[12px] text-slate-400">{cf.label||"—"}</span><span className="text-[12px] font-medium text-slate-700 text-right">{cf.type==="boolean"?selectedLead.customValues?.[cf.id]?"Yes":"No":selectedLead.customValues?.[cf.id]||"—"}</span></div>)}
                  </div>
                </div>
              )}

              <div className="bg-slate-50 rounded-xl p-4">
                <p className="text-[11px] text-slate-400 uppercase tracking-wider font-medium mb-3">Move to stage</p>
                <div className="flex gap-2 flex-wrap">
                  {stages.filter(s=>s.id!==selectedLead.stage).map(s=>(
                    <button key={s.id} onClick={()=>moveLead(selectedLead.id,s.id)} className="flex items-center gap-1.5 text-[12px] font-medium px-3 py-1.5 rounded-lg border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600 transition-all">
                      <span className="w-2 h-2 rounded-full" style={{background:s.color}}/>{s.label}<ChevronRight size={11}/>
                    </button>
                  ))}
                </div>
              </div>
      ...
      {showPermissions && <UserPermissionsModal users={users} setUsers={setUsers} onClose={()=>setShowPermissions(false)} />}

      {/* ── DELETE STAGE MODAL ── */}
      {deletingStage && (
        <DeleteStageModal
          stage={deletingStage}
          leadCount={leads.filter(l => l.stage === deletingStage.id).length}
          leadValue={leads.filter(l => l.stage === deletingStage.id).reduce((s, l) => s + l.value, 0)}
